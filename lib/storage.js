...
const VALID_TICKET_STATUSES = ["pending", "approved", "denied", "rejected", "sent"];
...
export async function updateTicketStatus(ticketId, status, reviewer, options = {}) {
  ...
  if (nextStatus === "sent" && !["approved", "sent"].includes(existingTicket.status)) {
    const error = new Error("Accept the request before marking the item as sent");
    error.statusCode = 409;
    throw error;
  }

  const reviewed = DECISION_STATUSES.includes(nextStatus);
  const sent = nextStatus === "sent";
  const now = new Date().toISOString();
  const reviewedAt = reviewed ? now : existingTicket.reviewedAt;
  const reviewedByDiscordId = reviewed ? reviewer.id : existingTicket.reviewedByDiscordId;
  const reviewedByUsername = reviewed ? displayDiscordName(reviewer) : existingTicket.reviewedByUsername;
  const decisionReason = reviewed ? normalizeDecisionReason(options.decisionReason) : existingTicket.decisionReason;
  const sentAt = sent ? now : null;
  const sentByDiscordId = sent ? reviewer.id : null;
  const sentByUsername = sent ? displayDiscordName(reviewer) : null;
  ...
  if (getStorageMode() === "postgres") {
    ...
    const result = await sql`
      ...
      SET status = ${nextStatus},
          ...
          sent_at = ${sentAt},
          sent_by_discord_id = ${sentByDiscordId},
...
async function ensureSchema(sql) {
  ...
  await sql`ALTER TABLE vplf_tickets ADD COLUMN IF NOT EXISTS sent_at TEXT`;
  await sql`ALTER TABLE vplf_tickets ADD COLUMN IF NOT EXISTS sent_by_discord_id TEXT`;
  await sql`ALTER TABLE vplf_tickets ADD COLUMN IF NOT EXISTS sent_by_username TEXT`;
...
function makeTicketCode(size = 6) {
  return `VCFL-${crypto.randomBytes(size).toString("hex").slice(0, size).toUpperCase()}`;
}
...
function rowToTicket(row) {
  ...
  return {
    ...
    sentAt: row.sent_at,
    sentByDiscordId: row.sent_by_discord_id,
    sentByUsername: row.sent_by_username,
...
function normalizeLocalTicket(ticket) {
  ...
  return {
    ...
    sentByDiscordId: ticket.sentByDiscordId || null,
    sentByUsername: ticket.sentByUsername || null,
    batchCode,
